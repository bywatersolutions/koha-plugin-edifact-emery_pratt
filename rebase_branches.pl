...
say "\nNot koha-plugin-edifact-emery-pratt, exiting" && exit 0 unless $repo eq "koha-plugin-edifact-emery-pratt";
...
foreach my $repo (@repos) {
    next if $repo eq 'koha-plugin-edifact-emery-pratt';
    next if $repo eq 'koha-plugin-edifact-emery-pratt-docs';
